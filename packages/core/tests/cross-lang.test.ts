...
const EXTENDED_VECTORS_GENERATED_AT = '2026-03-25T20:48:01.976Z';
const UPDATE_EXTENDED_CRYPTO_VECTORS = process.env.UPDATE_EXTENDED_CRYPTO_VECTORS === '1';
...
function buildExtendedVectorFile(vectors: ExtendedVector[]) {
  return {
    ...
    generated_at: EXTENDED_VECTORS_GENERATED_AT,
    algorithms: {
...
describe('Cross-Language: Generate extended-crypto.json (50 new vectors)', () => {
  ...
  beforeAll(async () => {
    ...
    async function addVector(id: string, description: string, message: Uint8Array, edgeCase?: string): Promise<void> {
      const seed = createHashSync(new TextEncoder().encode(`dot-protocol-r854-extended-vector:${id}`));
  ...
  it('extended-crypto.json matches generated deterministic vectors', () => {
    const output = buildExtendedVectorFile(vectors);

    if (UPDATE_EXTENDED_CRYPTO_VECTORS) {
      mkdirSync(TEST_VECTORS_DIR, { recursive: true });
      writeFileSync(EXTENDED_VECTORS_PATH, `${JSON.stringify(output, null, 2)}\n`, 'utf-8');
    }
